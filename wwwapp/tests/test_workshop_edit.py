...
class WorkshopEditViews(TestCase):
    ...
    def test_create_proposal_unauthenticated(self):
        response = self.client.get(reverse('workshops_add'))
        self.assertRedirects(response, reverse('login')+'?next='+reverse('workshops_add'))

        response = self.client.post(reverse('workshops_add'), {
