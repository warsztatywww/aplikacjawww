...
from .models import Workshop, Camp
...
def _get_emails_of_lecturers_of_workshops(workshops):
    users = set()
    for workshop in workshops:
        for user_profile in workshop.lecturer.all():
            users.add(user_profile.user.email)
    return users
...
def _all_lecturers(year):
    ...
    return _get_emails_of_lecturers_of_workshops(all_workshops)
...
def _accepted_lecturers(year):
    ...
    return _get_emails_of_lecturers_of_workshops(accepted_workshops)
...
def _denied_lecturers(year):
    ...
    return _get_emails_of_lecturers_of_workshops(denied_workshops)


@_register_as_email_filter('allParticipants', 'wszyscy uczestnicy zainteresowani tegoroczną edycją')
def _all_participants(year):
    return set([profile.user_profile.user.email for profile in
            year.participants.all()]) | set([interested.email for interested in year.interested_via_email.all()]) - _all_lecturers(year)
...
def _all_not_registered_participants(year):
    return set([profile.user_profile.user.email for profile in
            year.participants.all() if not profile.workshop_participation.exists()]) | _all_interested_emails_without_account(year) - _all_lecturers(year)


@_register_as_email_filter('allInterestedEmailsWithoutAccount', 'wszyscy uczestnicy którzy zapisali się na tegoroczną edycję za pomocą maila, ale jeszcze nie utworzyli konta')
def _all_interested_emails_without_account(year):
    return set([interested.email for interested in year.interested_via_email.all()]) - set([profile.user_profile.user.email for profile in
            year.participants.all()]) - _all_lecturers(year)
...
def _all_registered_participants(year):
    ...
    for workshop in all_workshops:
        for participant in workshop.participants.all():
            registered_users.add(participant.camp_participation.user_profile.user.email)
    return registered_users - _all_lecturers(year)
...
def filtered_emails_view(request, year):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            context['filtered_emails'] = form.filter_method(year)
    else:
