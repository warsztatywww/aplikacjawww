...
def workshop_solution(request, year, name, solution_id=None):
    ...
    if not solution_id:
        # My solution
        try:
            workshop_participant = workshop.workshopparticipant_set.get(participant__user=request.user)
        ...
        if not has_perm_to_edit and not request.user.has_perm('wwwapp.see_all_workshops'):
            return HttpResponseForbidden()
        solution = get_object_or_404(Solution.objects.filter(workshop_participant__workshop=workshop), pk=solution_id)
